...
function parseToken(lexeme) {
    ...
    const operatorToTokenType = {
        '+': TokenType.Plus,
        '-': TokenType.Minus,
        '*': TokenType.Asterisk,
        '/': TokenType.Slash,
    };
    if (Regex.isNumber(trimmedLexeme))
        return { type: TokenType.Number, lexeme: trimmedLexeme };
    if (Regex.isOperator(trimmedLexeme))
        return { type: operatorToTokenType[trimmedLexeme], lexeme: trimmedLexeme };
