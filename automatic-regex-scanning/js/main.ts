...
function parseToken(lexeme: string): Token {
    ...
    const operatorToTokenType: Record<Operator, TokenType> = {
        '+': TokenType.Plus,
        '-': TokenType.Minus,
        '*': TokenType.Asterisk,
        '/': TokenType.Slash,
    ...
    if (Regex.isNumber(trimmedLexeme)) return { type: TokenType.Number, lexeme: trimmedLexeme };
    if (Regex.isOperator(trimmedLexeme)) return { type: operatorToTokenType[trimmedLexeme], lexeme: trimmedLexeme };
...
class Regex {
    ...
    static isNumber(token: string): boolean {
        return Regex.number.test(token);
    }

    static isOperator(token: string): token is Operator {
        return Regex.operator.test(token);
    }
}